...
/umlflow scenarios                # one sequence diagram per use case (login, checkout, get-order, …)
...
/umlflow validate                 # check diagrams against the code evidence
...
Names are always clean — uncertainty is reported in each diagram's analysis notes and by `umlflow validate`,
never welded onto a name.
